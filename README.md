...
Built for the MunichTech EXPO Hackathon. It is aimed at teams and businesses (B2B) that want to connect their own data to an AI assistant safely, and at blind and low-vision developers who need non-visual feedback. The product is sold per generated server ($5 files, $15 database, $20 API), with no subscription. Payments run in test mode in this version.
...
- [Using a generated server](#using-a-generated-server)
- [Tests](#tests)
...
| `.zip` download | Real | Built in the browser from the generated files. Every download is counted on the usage page |
| Generated servers | Real | Compiled with TypeScript and exercised through a real MCP client in the test suite |
| Payment (card and PayPal) | Test mode | Simulated. No card data or PayPal login is collected or sent anywhere. The order itself is recorded, with the price computed on the server |
...
| Terms, privacy and refund text | Demo | Text written for this project, not reviewed by a lawyer |
...
Database servers are tested against simulated drivers that log every statement, not against a live PostgreSQL or MySQL instance. The SQL guard and the read-only transaction are verified (the suite includes attack cases such as whole-row references, subqueries in FROM and comma joins), but a full run against a real database is on the roadmap. The API server is tested against a local test upstream, including encoded slashes, NUL bytes and unreachable hosts.
...
- Validates and clamps that policy, then renders a complete project from verified templates: `package.json`, `tsconfig.json`, `src/index.ts`, `mcp.config.json`, `.env.example`, `.gitignore`, an MIT `LICENSE` and a README (the database server also carries a small `src/shims.d.ts`), so 8 or 9 files depending on the source.
...
- Ships a static website in English, German and Spanish with a quote builder, a demo checkout (card and PayPal), a result page with a real download and a receipt, a support page, a live usage page and documentation.
- Emails a receipt with the server attached (Resend), stores support messages with a ticket number and shows real usage numbers backed by PostgreSQL.
...
Voice: a service calls ElevenLabs text-to-speech (`eleven_multilingual_v2`) on demand and returns the MP3. A backup key is used when the main one is rejected or out of credits, repeated sentences are cached, and a daily ceiling protects the quota.
...
Usage numbers and support: events are kept in memory and, when `DATABASE_URL` is set, in PostgreSQL (Render), so they survive restarts. The public summary never contains personal data. Support messages are stored behind an admin token.

Email: receipts go out through the Resend HTTPS API from a verified domain (the free Render plan blocks SMTP), with the server .zip attached. SMTP is supported as an alternative.
...
4. The backend renders the project from the templates, records the build (and the order, with server-side prices) and returns the files, the rules and a spoken summary.
...
- Database: a single `SELECT` only, forbidden keywords refused, only exposed tables readable, blocked columns never returned (also inside JSON values), structures that could bypass those lists refused, and execution inside a read-only transaction with a time limit.
- API: only listed endpoints can be called, full URLs, redirects and encoded slashes or dots are refused, blocked JSON fields are removed, connection problems are reported in plain words and long responses are truncated.
...
The `.zip` is a standard MCP server that runs on your own machine or server, so your data never passes through MCP Builder.

```bash
unzip server.zip -d my-server && cd my-server
npm install
npm run build
```

Then add it to your MCP client. For Claude Desktop, add the entry from the `mcp.config.json` inside the zip to `claude_desktop_config.json`, with the absolute path to `dist/index.js` and the environment variables listed in `.env.example` (folder paths, database URL or API base URL). Cursor and Windsurf use the same format. Restart the client and ask it a question about your data. The README inside the zip lists the tools, the rules that were applied and the connection steps.

Note for the database server: a blocklist is a second layer. Give it a database user that can only read the tables it needs.
...
- 87 tests in total: 79 unit tests and 8 end-to-end tests. The unit tests cover the policy sanitizing (hostile values are clamped and cleaned), the project renderer, the localized rules, the API contract, the ElevenLabs backup key, the receipt email (server-side amounts, one email per build, no leaked secrets), the usage numbers, the support inbox and its token, the PostgreSQL storage with a fallback to memory, the request size limit and the caps that do not depend on the caller address.
- End-to-end tests render real projects, install their dependencies, compile them with TypeScript and talk to them through the official MCP client over stdio. They check that path escapes are refused in the files server, that the API server calls only allowed endpoints, rejects encoded path tricks and reports upstream errors, and that the database servers run inside a read-only transaction and reject unsafe SQL, including column and table allowlist bypass attempts (against simulated drivers).
...
MCP-Builder-Munichtech/
  ...
  submission/                  Devpost text, story, video script, checklist and gallery images
  generator/
  ...
  tests/
    ...
    test_metrics.py            Usage numbers, support inbox and storage tests
    test_receipts.py           Receipt email tests
  ...
  site/                        Static website, the only folder that is published
    ...
    support.html               Support page (real form, gives a ticket number)
    politics.html              Terms, privacy and refunds (project text)
    admin.html                 Live usage page (real numbers) and the private support inbox
...
| GET | `/api/health` | Service and integration status: model, voice, email, persistent metrics, proxy hops |
...
| 429 | `rate_limited` | Too many requests from this client, or from everyone together |
| 429 | `daily_limit` | The daily ceiling of the demo was reached |
| 411 | `length_required` | A `Content-Length` header is required on writes |
| 413 | `payload_too_large` | The request body is larger than `MAX_BODY_BYTES` |
| 401 | `unauthorized` | Missing or wrong admin token |
| 503 | `admin_disabled` | No `ADMIN_TOKEN` is configured, so the inbox is off |
...
- The option to run the AI step on a self-hosted open-weight model.
...
1. API on Render: create a Blueprint from this repository (it reads `render.yaml`, which also creates the free PostgreSQL database and links it as `DATABASE_URL`). Enter the secrets when asked: `FEATHERLESS_API_KEY`, `ELEVENLABS_API_KEY` (and optionally `ELEVENLABS_API_KEY_2`), `RESEND_API_KEY`, `MAIL_FROM`, `ADMIN_TOKEN` and `SUPPORT_TO`. Check `/api/health`, then add the custom domain `mcpbuilder-api.quevedojose.com`.
   - Email: verify your sending domain in Resend (add its DNS records) and use an address on it in `MAIL_FROM`.
   - Proxy: Render adds one `X-Forwarded-For` entry, so `TRUSTED_PROXY_HOPS` defaults to `1`. If you put another proxy such as Cloudflare in front, set it to `2`. `/api/health` shows `proxy_hops` and `forwarded_entries` to check it.
   - The free Render database expires after a fixed period, so export the data or upgrade the plan for long-term use.
