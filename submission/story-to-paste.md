...
Target users (B2B): teams and businesses that want to connect their own data to an AI assistant safely, without weeks of custom integration work, and blind or low-vision developers who need non-visual feedback. The product is sold per generated server, so a company pays only for the source it needs.

How it is used in practice: the .zip is a standard MCP server. Unzip it, run `npm install` and `npm run build`, and point any MCP client (Claude Desktop, Cursor, Windsurf) at it. It runs on the customer's own machine or server, so their data never passes through MCP Builder.
...
- Business (B2B): one price per generated server ($5 files, $15 database, $20 API), with no subscription. Payments run in test mode in this version.
- Scalability: the API is stateless, the usage numbers live in PostgreSQL, and the generated servers run on the customer's side, so serving more customers does not add load on their data.
